...
function onLoginSuccess() {
    ...
    // Kiểm tra xem có trang nào cần redirect không
    const redirectTarget = sessionStorage.getItem('authRedirectTarget');
    
    if (redirectTarget) {
        // Xóa target để không bị lặp lại
        sessionStorage.removeItem('authRedirectTarget');
        // Quay lại đúng trang người dùng muốn vào
        window.location.href = redirectTarget;
    } else {
        // Nếu không có trang nào cần redirect → về trang mặc định (home hoặc dashboard)
